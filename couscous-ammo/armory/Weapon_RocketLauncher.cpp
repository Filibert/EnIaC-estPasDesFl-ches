...
void RocketLauncher::InitializeFuzzyModule()
{
  ...
  FzSet& Target_VeryClose = DistToTarget.AddLeftShoulderSet("Target_VeryClose",0,25,125);
  FzSet& Target_Close = DistToTarget.AddTriangularSet("Target_Close", 25, 125, 225);
  FzSet& Target_Medium = DistToTarget.AddTriangularSet("Target_Medium",125,225,325);
  FzSet& Target_Far = DistToTarget.AddTriangularSet("Target_Far", 225, 325, 425);
  FzSet& Target_VeryFar = DistToTarget.AddRightShoulderSet("Target_VeryFar",325,425,1000);

  FuzzyVariable& Desirability = m_FuzzyModule.CreateFLV("Desirability"); 

  FzSet& AbsolutelyDesirable = Desirability.AddRightShoulderSet("AbsolutelyDesirable", 65, 80, 100);
  FzSet& VeryDesirable = Desirability.AddTriangularSet("VeryDesirable", 50, 65, 80);
  FzSet& Desirable = Desirability.AddTriangularSet("Desirable", 35, 50, 65);
  FzSet& SomehowDesirable = Desirability.AddTriangularSet("SomehowDesirable", 20, 35, 50);
  FzSet& Undesirable = Desirability.AddLeftShoulderSet("Undesirable", 0, 20, 35);

  FuzzyVariable& AmmoStatus = m_FuzzyModule.CreateFLV("AmmoStatus");

  FzSet& Ammo_Loads = AmmoStatus.AddRightShoulderSet("Ammo_Loads", 50, 60, 100);
  FzSet& Ammo_Good = AmmoStatus.AddTriangularSet("Ammo_Good", 25, 40, 55);
  FzSet& Ammo_Okay = AmmoStatus.AddTriangularSet("Ammo_Okay", 12.5, 25, 40);
  FzSet& Ammo_Low = AmmoStatus.AddTriangularSet("Ammo_Low", 5, 12.5, 20);
  ...
  m_FuzzyModule.AddRule(FzAND(Target_Close, Ammo_Good), SomehowDesirable);
  m_FuzzyModule.AddRule(FzAND(Target_Close, Ammo_Okay), SomehowDesirable);
  ...
  m_FuzzyModule.AddRule(FzAND(Target_Medium, Ammo_Loads), AbsolutelyDesirable);
  m_FuzzyModule.AddRule(FzAND(Target_Medium, Ammo_Good), AbsolutelyDesirable);
  ...
  m_FuzzyModule.AddRule(FzAND(Target_Medium, Ammo_VeryLow), SomehowDesirable);

  m_FuzzyModule.AddRule(FzAND(Target_Far, Ammo_Loads), Desirable);
  m_FuzzyModule.AddRule(FzAND(Target_Far, Ammo_Good), SomehowDesirable);
  m_FuzzyModule.AddRule(FzAND(Target_Far, Ammo_Okay), SomehowDesirable);
  ...
  m_FuzzyModule.AddRule(FzAND(Target_VeryFar, Ammo_Loads), SomehowDesirable);
