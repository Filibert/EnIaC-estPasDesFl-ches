...
void Raven_Game::Update()
{
	...
	if (m_bRemoveABot)
	{
		if (!m_Bots.empty())
		{
			...
			if (m_bTeamMode)
			{
				pBot->GetTeam()->RemoveTeammate(pBot);
			}
			delete m_Bots.back();
			m_Bots.remove(pBot);
			pBot = 0;
...
//-----------------------------------------------------------------------------
void Raven_Game::AddBots(unsigned int NumBotsToAdd, bool isLearning)
{
	while (NumBotsToAdd--)
	{
		Raven_Team* team = 0;
		if (m_bTeamMode) {
			...
			for (curTeam; curTeam != m_Teams.end(); curTeam++)
			{
				...
				if (t->GetTeamSize() <= minNbMembers || minNbMembers == -1)
				{
					minNbMembers = t->GetTeamSize();
					team = t;
				}
			}
		}
		...
		if (isLearning) rb->SetLearning(true);
		team->AddTeammate(rb);
...
bool Raven_Game::LoadMap(const std::string& filename)
{
	...
	if (m_pMap->LoadMap(filename))
	{
		...
		for (int i = 0; i < teamCount; ++i) {
			m_Teams.push_back(new Raven_Team(i, this, i % Raven_Team::NB_BEHAVIOURS));
		}
...
//------------------------- ChangeTeamMode ---------------------------
//
//  when called will release any bot from their team or create new teams
//-----------------------------------------------------------------------------
...
//------------------------- ChangeTeamFlocking ---------------------------
//
//  when called activate or deactivate the team flocking
//-----------------------------------------------------------------------------
void Raven_Game::ChangeTeamFlocking(bool flocking) {
	m_bTeamFlocking = flocking;
	if (flocking)
	{
...
//------------------------- Move ---------------------------
//
//  allow to move the possessed bot
//-----------------------------------------------------------------------------
void Raven_Game::Move(char c)
{
	if (m_pSelectedBot == NULL) return;
	Vector2D mouvement = Vector2D(0, 0);
	bool canStep = false;
	switch (c)
	{
	case 'u':
		mouvement = Vector2D(0, -10);
		break;
	case 'l':
		mouvement = Vector2D(-10, 0);
		break;
	case 'r':
		mouvement = Vector2D(10, 0);
		break;
	...
	canStep = m_pSelectedBot->canWalkTo(m_pSelectedBot->Pos() + mouvement);
	if (m_pSelectedBot->isPossessed() && canStep)
	{
		m_pSelectedBot->SetVelocity(mouvement);
	}
}
...
void Raven_Game::ClickRightMouseButton(POINTS p)
{
	...
	if (pBot && pBot == m_pSelectedBot)
	{
		...
		human_player = m_pSelectedBot;
		HasAHumanPlayer = true;
