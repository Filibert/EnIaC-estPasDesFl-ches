...
#include "triggers/Trigger_WeaponDropper.h" 
...
void Raven_Map::AddWeaponDropTrigger(Vector2D pos, unsigned int weapon, int ammo, int team, Raven_Game* game) {
	int closestNode_id = GetClosestNodeToPositionGeneric(pos);

	Trigger_WeaponDropper* wd = (new Trigger_WeaponDropper(pos, weapon, ammo, team, game, closestNode_id));
	
	//add it to the appropriate vectors
	m_TriggerSystem.Register(wd);

	//let the corresponding navgraph node point to this object
	NavGraph::NodeType& node = m_pNavGraph->GetNode(wd->GraphNodeIndex());

	node.SetExtraInfo(wd);

	//register the entity
	EntityMgr->RegisterEntity(wd);
}
...
//
//  returns the index of the closest graph node to the given position
//-----------------------------------------------------------------------------
int Raven_Map::GetClosestNodeToPositionGeneric(Vector2D pos)const
{
