...
void Raven_WeaponSystem::KillThemAll()const
{
			...
			m_dAimPersistance))
	{
		...
		Raven_Bot* enemyBot = m_pOwner->GetTargetBot();
		int enemyHealth = enemyBot->Health();
		double distance = m_pOwner->Pos().Distance(enemyBot->Pos());
		unsigned int weaponType = m_pOwner->GetWeaponSys()->GetCurrentWeapon()->GetType();
		unsigned int remainingAmmo = m_pOwner->GetWeaponSys()->GetAmmoRemainingForWeapon(weaponType);
		int isVisible = true;
		Vector2D AimingPos = m_pOwner->GetTargetBot()->Pos();

		float frequencies[5] = { weaponType, remainingAmmo, isVisible, enemyHealth, distance };
...
void Raven_WeaponSystem::AddNoiseToAim(Vector2D& AimingPos)const
{
  ...
  Vector2D noise = m_pOwner->GetNoiseSystem()->GetNoise(); // Add fuzzy functions   

  AimingPos = toPos + m_pOwner->Pos() + noise;
}
