...
Raven_Steering::Raven_Steering(Raven_Game* world, Raven_Bot* agent):
             ...
             m_dWeightSeparation(script->GetDouble("SeparationWeight")),
		   	 m_dWeightCohesion(20),//script->GetDouble("CohesionWeight")), CohesiontWeight = 0.1; does not work in params.lua
			 m_dWeightAlignment(10), //script->GetDouble("AlignmentWeight")), AlignmentWeight = 10;
...
//---------------------------- Cohesion --------------------------------
//
// this calculates a force attracting to the barycenter of the other neighbors
//------------------------------------------------------------------------
Vector2D Raven_Steering::Cohesion(const std::list<Raven_Bot*> &agents)
{
	...
	for (it; it != agents.end(); ++it)
	{
		...
		//include the evade target ***
		if ((*it != m_pRaven_Bot) && (*it)->IsTagged() &&
			(*it != m_pTargetAgent1) && (!m_pWorld->IsTeamModeOn() || (*it)->GetTeam() == m_pRaven_Bot->GetTeam()))
		{
			CenterOfMass += (*it)->Pos();
...
//---------------------------- Alignment --------------------------------
//
// this calculates a force attractingto the alignment of the other neighbors
