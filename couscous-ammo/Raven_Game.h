...
private:
  ...
  //if true enable the teams
  bool	                           m_bTeamMode;

  //if true enable the flocking
  bool m_bTeamFlocking;
...
public:
  ...
  bool HasAHumanPlayer = false;
  Raven_Bot* human_player = NULL;
  boolean IsInTrainingMode() { return training; };
  void SetTrainingMode(boolean b) { training = b; };

  //loads an environment from a file
  bool LoadMap(const std::string& FileName); 

  void AddBots(unsigned int NumBotsToAdd, bool isLearning = false);
  ...
  //activatet the teal
  void ChangeTeamFlocking(bool flocking);

  //returns true if team mode is on
  bool IsTeamModeOn() { return m_bTeamMode; }

  bool IsTeamFlockingOn() { return m_bTeamFlocking; }
