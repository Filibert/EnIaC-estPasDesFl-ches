...
std::fstream myWrittingFile;
std::fstream myReadingFile;
std::string data = "";
...
int isVisible = 0;
unsigned int weapon;
unsigned int remainingAmmo;
double distance = 0;
//nb jeux de valeurs observable
int i = 0;
	...
	LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		CheckMenuItemAppropriately(hwnd, IDM_BOTS_SHOW_STRATEGIES, UserOptions->m_bShowBotStrategies);
	...
	case WM_KEYUP:
	{
		...
		case 'T':
			...
			g_pRaven->ChangeTeamMode(!g_pRaven->IsTeamModeOn());

			break;

		case 'F':

			g_pRaven->ChangeTeamFlocking(!g_pRaven->IsTeamFlockingOn());

			if (g_pRaven->IsTeamFlockingOn())
				debug_con << "Team Flocking is On" << "";
			else
				debug_con << "Team Flocking is Off" << "";
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (g_pRaven->HasAHumanPlayer) {
			i++;
			weapon = g_pRaven->human_player->GetWeaponSys()->GetCurrentWeapon()->GetType();
			remainingAmmo = g_pRaven->human_player->GetWeaponSys()->GetAmmoRemainingForWeapon(weapon);

			(g_pRaven->GetAllBotsInFOV(g_pRaven->human_player).size() > 0) ? isVisible = 1 : isVisible = 0;

			if (isVisible)
			{
				botLife = g_pRaven->GetAllBotsInFOV(g_pRaven->human_player)[0]->Health();
				distance = (g_pRaven->human_player->Pos()).Distance(g_pRaven->GetAllBotsInFOV(g_pRaven->human_player)[0]->Pos());
			}
			data += std::to_string(weapon) + " "
				+ std::to_string(remainingAmmo) + " "
				+ std::to_string(isVisible) + " "
				+ std::to_string(botLife) + " "
	...
	case WM_COMMAND:
	{
		...
		case IDM_GAME_TRAINNING_MODE:
			
			g_pRaven->SetTrainingMode(!g_pRaven->IsInTrainingMode());
			CheckMenuItemAppropriately(hwnd, IDM_GAME_TRAINNING_MODE, g_pRaven->IsInTrainingMode());
		...
		case IDM_BOTS_SHOW_STRATEGIES:

			UserOptions->m_bShowBotStrategies = !UserOptions->m_bShowBotStrategies;
...
//make sure the window creation has gone OK
		...
		myReadingFile.open("data_training.txt", std::fstream::in);
		if(!myReadingFile.peek() == std::fstream::traits_type::eof())
			myReadingFile.seekg(0, std::ios_base::beg);
			data.assign((std::istreambuf_iterator<char>(myReadingFile)),
				(std::istreambuf_iterator<char>()));
		
		if (!data.empty()) {
			i = std::stoi(data.substr(0, str.find(" ")));
		...
		while (!bDone)
		{
			...
			{
				g_pRaven->Update();
				Raven_Bot* player = g_pRaven->human_player;

				if (g_pRaven->HasAHumanPlayer && fmod(timer.CurrentTime(), 2) <= 0.02 && g_pRaven->IsInTrainingMode()) {
					i++;
					weapon = player->GetWeaponSys()->GetCurrentWeapon()->GetType();
					remainingAmmo = player->GetWeaponSys()->GetAmmoRemainingForWeapon(weapon);

					(g_pRaven->GetAllBotsInFOV(player).size() > 0) ? isVisible = 1 : isVisible = 0;

					if (isVisible)
					{
						botLife = g_pRaven->GetAllBotsInFOV(player)[0]->Health();
						distance = (player->Pos()).Distance(g_pRaven->GetAllBotsInFOV(player)[0]->Pos());
					}
					data += std::to_string(weapon) + " "
						+ std::to_string(remainingAmmo) + " "
						+ std::to_string(isVisible) + " "
		...
		myWrittingFile.open("data_training.txt", std::fstream::out | std::ofstream::trunc);
		myWrittingFile << i << " 5 1" << std::endl << std::endl << data;
		myWrittingFile.close();
		if (g_pRaven->IsInTrainingMode()) {
			struct fann *ann = fann_create_standard(2, 5, 1);
			fann_train_on_file(ann, "data_training", 200,
				10, 0.1);
