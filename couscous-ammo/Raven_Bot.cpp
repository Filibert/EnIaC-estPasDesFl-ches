           ...
           
{
  ...
  m_pWeaponNoiseSystem = new Raven_NoiseSystem(this);
}
...
void Raven_Bot::ReduceHealth(unsigned int val)
{
	...
	if (GetWorld()->IsTeamModeOn())
	{
		if (m_iHealth <= 0)
		{
			...
			std::list<int> ids = this->GetTeam()->GetTeammatesIds();
...
//---------------------------- DropWeapon -------------------------------------
//
//  Drop the current weapon at the current position
//-----------------------------------------------------------------------------
void Raven_Bot::DropWeapon() {
	//Blaster gun is the default weapon and has unlimited ammo so useless to drop it
	if (m_pWorld->IsTeamModeOn() && m_pWeaponSys->GetCurrentWeapon()->GetType() != type_blaster) {
		// Choose a position to drop the current weapon
		double theta = 2 * RandFloat() * pi;
		int radius = RandInt(30, 50);
		Vector2D offset(radius * cos(theta), radius * sin(theta));
		Vector2D pos = m_pWorld->GetMap()->GetSpawnPoints().at(m_pTeam->GetId()) + offset;

		// Drop the current weapon
		Raven_Weapon* w = m_pWeaponSys->GetCurrentWeapon();
		debug_con << "Bot " << this->ID() << "dropped a weapon " << w->GetType() << "";
		m_pWorld->GetMap()->AddWeaponDropTrigger(pos, w->GetType(), w->NumRoundsRemaining(), m_pTeam->GetId(), m_pWorld);

		// Alert the team
		m_pTeam->AddDroppedWeapon(pos);
	}
}
...
void Raven_Bot::Render()                                         
{
  ...
  if (m_pWorld->IsTeamModeOn())
	  Raven_Team::TeamColorBrush(m_pTeam->GetId());
  else
	  gdi->BrownBrush();
  ...
  if (UserOptions->m_bShowBotStrategies)
  {
	 gdi->TextAtPos(Pos().x + 10, Pos().y - 20, ttos(GetTeam()->Behaviour()));
  }
}
